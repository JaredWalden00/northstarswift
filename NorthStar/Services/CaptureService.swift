...
/// Service to request a remote server to capture a photo and return it.
///
/// Supports two response formats:
/// 1. **Raw image bytes** — server returns the image directly (Content-Type: image/*)
/// 2. **JSON with base64** — server returns `{ "image_b64": "..." }` or `{ "image": "..." }`
